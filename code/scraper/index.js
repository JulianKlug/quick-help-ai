...
function saveResults({ errors, results }) {
  ...
  `);
  fs.writeFileSync(
    __dirname + `/data/results/results.json`,
    JSON.stringify(results, null, 2)
  );
  fs.writeFileSync(
    __dirname + `/data/errors/error.json`,
...
async function pageEvaluation() {
  ...
  discussion.title = document.querySelector(
    '[class="DiscussionHero-title"]'
  ).textContent;
  ...
  discussion.question = getPostText(firstPost);
  discussion.questionUsername = getUsername(firstPost);
  ...
  function isPostFromEmployee(postNode) {
    const badgeWrapper = postNode.querySelector(
      '[class="PostUser-badges badges"]'
    );
    ...
    const rolesOfPoster = Array.from(badges).map((span) =>
      span.getAttribute("data-original-title")
    );
    const isEmployee =
      rolesOfPoster.includes("Admin") ||
      rolesOfPoster.includes("Community Manager");
    return !!isEmployee;
  }

  function getPostText(postNode) {
    return postNode
      .querySelector('[class="Post-body"]')
      .innerText.replace("\n", "");
  }

  function getUsername(postNode) {
    return postNode.querySelector('[class="username"]').textContent;
  }
}
